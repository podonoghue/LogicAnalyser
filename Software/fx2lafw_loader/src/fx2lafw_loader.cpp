...
enum EepromSize {
   EepromSize_Small = bREQUEST_SMALL_EEPROM_LOAD,
   EepromSize_Large = bREQUEST_LARGE_EEPROM_LOAD,
};
...
static constexpr unsigned MAX_BDM_DEVICES = 10;
...
bool start_libusb() {
   ...
   for (unsigned i=0; i<=MAX_BDM_DEVICES; i++) {
...
void downloadFirmwareToTargetRam(libusb_device_handle *usbDeviceHandle, HexImage &firmware_image) {
   downloadToTargetRam(usbDeviceHandle, 0, firmware_image.image_size, firmware_image.toArray());
}
...
void uploadFirmwareFromTargetRam(libusb_device_handle *usbDeviceHandle, HexImage &firmware_image) {
   uploadFromTargetRam(usbDeviceHandle, 0x00, firmware_image.image_size, firmware_image.toArray());
}
...
static constexpr int MAX_IMAGE_SIZE        = 0x4000;
...
void uploadEepromFromTarget(libusb_device_handle *usbDeviceHandle, HexImage &eeprom_image, EepromSize eepromSize) {
   ...
   uint16_t length = eeprom_image.image_size;
...
void downloadEepromUtility(libusb_device_handle *handle) {
   ...
   HexImage firmware_image(MAX_IMAGE_SIZE);
   HexImage firmware_image_readback(MAX_IMAGE_SIZE);
...
//   firmware_image.print();
   ...
   if (firmware_image.compare(firmware_image_readback) != 0) {
      ...
      fprintf(stdout, "Firmware verify OK\n");
   }
...
void readEeprom(EepromTypes eepromType) {
   ...
   uploadEepromFromTarget(handle, eeprom_image, eepromType.eepromSize);
   ...
   eeprom_image.print();
   eeprom_image.saveHexFile("eeprom.hex");
...
void writeEeprom(EepromTypes eepromType) {
   ...
   uploadEepromFromTarget(handle, original_eeprom_image, eepromType.eepromSize);

   if ((original_eeprom_image[0] == 0xCD) && (original_eeprom_image[1] == 0xCD)) {
      throw MyException("Failed EEPROM read. Wrong image size type?");
   }
   original_eeprom_image.print();
   original_eeprom_image.saveHexFile("saved_eeprom_image.hex");

   HexImage new_eeprom_image(eepromType.size);
   new_eeprom_image.loadHexFile("new_eeprom_image.hex");

   downloadEepromToTarget(handle, new_eeprom_image,eepromType.eepromSize);

   HexImage verify_eeprom_image(eepromType.size);
   uploadEepromFromTarget(handle, verify_eeprom_image, eepromType.eepromSize);
...
//   original_eeprom_image[20] = 0xFF;
//   original_eeprom_image[21] = 0xFF;
//
//   downloadEepromToTarget(handle, original_eeprom_image,eepromType.eepromSize);
...
EepromTypes eepromTypes[] = {
      {"24LC01",      128,  EepromSize_Small },
      {"24LC02",      256,  EepromSize_Small },
      {"24LC02",      512,  EepromSize_Small },
      {"24LC64",   0x4000,  EepromSize_Large },
      {"M24128",   0x4000,  EepromSize_Large },
};

int main() {
   fprintf(stdout, "Starting\n"); fflush(stdout);
   try {
//      readEeprom(eepromTypes[0]);
      writeEeprom(eepromTypes[0]);
   }
   catch (MyException &e) {
